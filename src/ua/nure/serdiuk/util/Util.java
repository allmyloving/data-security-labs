package ua.nure.serdiuk.util;

import java.util.ArrayList;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

import ua.nure.serdiuk.SBox;
import ua.nure.serdiuk.Table;

public final class Util {

	public static final long BIT6_MASK = 0x3F;
	public static final long BIT32_MASK = 0xFFFF_FFFF;

	public static int[] fillInitial() {
		int[] table = new int[64];

		int sign = 5;
		for (int s = 57, c = 4; s <= 64; s++, c += sign) {
			for (int j = s, i = c * 8; j > 0; j -= 8, i++) {
				table[i] = j;
			}
			sign = (sign == 5) ? -4 : 5;
		}

		return table;
	}

	public static int[] fillC() {
		int[] table = new int[28];

		for (int s = 57, c = 0; s <= 60; s++, c++) {
			for (int j = s, i = c * 8; i < c * 8 + 8 && i < 28; j -= 8, i++) {
				table[i] = j;
			}
		}
		return table;
	}

	public static int[] fillD() {
		int[] table = new int[28];

		for (int s = 63, c = 0; s >= 61; s--, c++) {
			for (int j = s, i = c * 8; i < c * 8 + 8; j -= 8, i++) {
				table[i] = j;
			}
		}
		for (int i = 24, s = 28; i < 28; i++, s -= 8) {
			table[i] = s;
		}
		return table;
	}

	public static int[] fillFinal() {
		int[] table = new int[64];

		int sign = 40;
		for (int s = 40, c = 0; s >= 33; s--, c++) {
			for (int j = s, i = c * 8; i < c * 8 + 8; j += sign, i++) {
				table[i] = j;
				sign = sign == -32 ? 40 : -32;
			}
		}

		return table;
	}

	public static List<Byte> toByteList(byte[] arr) {
		List<Byte> bytes = new ArrayList<>();
		for (byte b : arr) {
			bytes.add(b);
		}
		return bytes;
	}

	private static final int[] S = { 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7, // S1
			0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8, 4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0,
			15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13, 15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10, // S2
			3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5, 0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15,
			13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9, 10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8, // S3
			13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1, 13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7,
			1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12, 7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15, // S4
			13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9, 10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4,
			3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14, 2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9, // S5
			14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6, 4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14,
			11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3, 12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11, // S6
			10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8, 9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6,
			4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13, 4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1, // S7
			13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6, 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2,
			6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12, 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7, // S8
			1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2, 7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8,
			2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 };

	public static final SBox getSBox() {
		Table table;
		SBox box = new SBox();
		for (int i = 0, j = 0; i < 8; i++, j += 16 * 4) {
			table = new Table();
			for (int r = 0; r <= 3; r++) {
				for (int c = 0; c <= 15; c++) {
					table.add(r, c, S[j + r * 16 + c]);
				}
			}
			// System.out.println(table);
			box.add(i, table);
		}

		return box;
	}

	public static long getBit(long input, int bitNumFrom) {
		return (input >> bitNumFrom) & 1;
	}

	public static long cycleShiftLeft(long num, int shifts) {
		long n = (num << shifts) | (num >> (28 - shifts));
		return n & 0xFFF_FFFF;
	}
}
